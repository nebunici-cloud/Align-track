  ...
  return {
    ...
    build: {
      rollupOptions: {
        output: {
          manualChunks: {
            // Firebase (auth+firestore+storage) is the single largest
            // dependency and changes far less often than app code — its own
            // chunk means a normal app-code deploy doesn't invalidate the
            // browser's cache of it.
