...
//
// ── Setup (do this ONCE) ────────────────────────────────────────────────
// 1. Create a new script in Scriptable, name it "TrayceWidgetLockScreen",
//    and paste this whole file in - it REPLACES pasting
//    TrayceWidgetLockScreen.js directly.
...
// 3. Long-press the Lock Screen -> Customize -> Lock Screen -> tap a
//    widget slot -> Scriptable -> pick a style -> set "Script" to
//    TrayceWidgetLockScreen.
...
const RAW_URL =
  "https://raw.githubusercontent.com/nebunici-cloud/Align-track/claude/aligners-app-analysis-roadmap-agy1pn/scriptable/TrayceWidgetLockScreen.js";
...
async function getScriptCode() {
  ...
  const codePath = fm.joinPath(fm.cacheDirectory(), "trayce-widget-lockscreen-remote.js");
  const metaPath = fm.joinPath(fm.cacheDirectory(), "trayce-widget-lockscreen-remote.meta");
  ...
  try {
    ...
    if (!freshCode || freshCode.length < 150) {
...
function buildLoaderErrorWidget(message) {
  ...
  const text = widget.addText(`Trayce loader error`);
