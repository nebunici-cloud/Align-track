...
//
// ── Setup (do this ONCE) ────────────────────────────────────────────────
// 1. Create a new script in Scriptable, name it "TrayceWidget", and paste
//    this whole file in - it REPLACES pasting TrayceWidget.js directly.
...
// 3. Add it to your Home Screen as the small widget size, same as before.
...
const RAW_URL =
  "https://raw.githubusercontent.com/nebunici-cloud/Align-track/claude/aligners-app-analysis-roadmap-agy1pn/scriptable/TrayceWidget.js";

// Short while widgets are still being actively tweaked - safe to raise
// once the design settles, to avoid a network fetch on every tap/refresh.
const CACHE_MAX_AGE_MS = 5 * 60 * 1000;

async function getScriptCode() {
  const fm = FileManager.local();
  const codePath = fm.joinPath(fm.cacheDirectory(), "trayce-widget-remote.js");
  const metaPath = fm.joinPath(fm.cacheDirectory(), "trayce-widget-remote.meta");
